...
public final class CounterTestKit {
  ...
  public ValueEntityResult<Empty> reset(CounterApi.ResetValue resetValue, Metadata metadata) {
    entity._internalSetCommandContext(Optional.of(new TestKitValueEntityCommandContext(entityId, metadata)));
    entity._internalSetCurrentState(state);
    ValueEntity.Effect<Empty> effect = entity.reset(state, resetValue);
    return interpretEffects(effect);
  }

  public ValueEntityResult<CounterApi.CurrentCounter> getCurrentCounter(CounterApi.GetCounter getCounter, Metadata metadata) {
    entity._internalSetCommandContext(Optional.of(new TestKitValueEntityCommandContext(entityId, metadata)));
    entity._internalSetCurrentState(state);
    ValueEntity.Effect<CounterApi.CurrentCounter> effect = entity.getCurrentCounter(state, getCounter);
    return interpretEffects(effect);
  }
  ...
  public ValueEntityResult<Empty> reset(CounterApi.ResetValue resetValue) {
    entity ._internalSetCommandContext(Optional.of(new TestKitValueEntityCommandContext(entityId, Metadata.EMPTY)));
    ValueEntity.Effect<Empty> effect = entity.reset(state, resetValue);
    return interpretEffects(effect);
  }

  public ValueEntityResult<CounterApi.CurrentCounter> getCurrentCounter(CounterApi.GetCounter getCounter) {
    entity ._internalSetCommandContext(Optional.of(new TestKitValueEntityCommandContext(entityId, Metadata.EMPTY)));
    ValueEntity.Effect<CounterApi.CurrentCounter> effect = entity.getCurrentCounter(state, getCounter);
    return interpretEffects(effect);
  }
